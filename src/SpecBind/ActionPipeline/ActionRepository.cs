...
namespace SpecBind.ActionPipeline
{
    internal class ActionRepository : IActionRepository
    {
        ...
        public ActionRepository(IObjectContainer objectContainer, ISettings settings)
        {
        ...
        {
            var excludedAssemblyNames = this.settings.ExcludedAssemblies.Select(a => a.Name);
            // Get all items from the current assemblies
            var assemblies = AppDomain.CurrentDomain.GetAssemblies().Where(a => !a.IsDynamic && !a.GlobalAssemblyCache && !excludedAssemblyNames.Contains(a.FullName));
